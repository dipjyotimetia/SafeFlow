// CoinSpot Crypto Exchange Statement Parser
...
/**
 * Parser for CoinSpot statements (Australian crypto exchange)
 *
 * CoinSpot statements typically include:
 * - Date/Time
 * - Transaction type (Buy, Sell, Deposit, Withdrawal, Trade)
 * - Cryptocurrency symbol
 * - Amount in crypto
 * - AUD value
 * - Fees
 */
export class CoinSpotParser implements BankParser {
  name = 'CoinSpot';
  bankCode = 'coinspot';

  // Patterns to identify CoinSpot statements
  ...
  // Additional CoinSpot-specific skip patterns
  private readonly coinspotSkipPatterns = [
    /^Portfolio\s+Summary/i,
    /^Asset\s+Holdings/i,
    /^Tax\s+Report/i,
    /^Coin\s+Symbol\s+Amount/i,
  ];
  ...
  parse(text: string): ParseResult {
    ...
    if (transactions.length === 0) {
      errors.push('No transactions found in the document. Please ensure this is a valid CoinSpot statement.');
    }
  ...
  private parseTransactionLine(line: string, defaultYear: number): ParsedTransaction | null {
    ...
    // Skip CoinSpot-specific non-transaction lines
    if (this.coinspotSkipPatterns.some((pattern) => pattern.test(line))) {
      return null;
    }
    ...
    if (audMatch) {
      ...
      // Use the first reasonable amount (likely to be the trade value)
...
export const coinspotParser = new CoinSpotParser();
